<template>
  <div class="program-change-approved">
    <t-card title="方案变更已审核" :bordered="false">
      <template #actions>
        <t-row justify="end">
          <t-col>
            <t-space>
      ...
      >
        <template v-if="currentDetail">
          <t-descriptions bordered size="large">
            ...
            <t-descriptions-item label="审核时间">{{ formatDate(currentDetail.approveTime) }}</t-descriptions-item>
            <t-descriptions-item label="申请人">{{ currentDetail.creator }}</t-descriptions-item>
            <t-descriptions-item label="审核人">{{ currentDetail.approver }}</t-descriptions-item>
            ...
            <t-descriptions-item label="审核意见" span="2">
              <div class="long-text">{{ currentDetail.approveComment || '无' }}</div>
            </t-descriptions-item>
...
const columns = [
  ...
  { colKey: 'approver', title: '审核人', width: 100 },
  { colKey: 'createTime', title: '申请时间', width: 160 },
  { colKey: 'approveTime', title: '审核时间', width: 160 },
...
const fetchTableData = async () => {
  try {
    ...
    const mockData = Array.from({ length: 20 }).map((_, index) => ({
      ...
      approver: `审核员${index % 2 + 1}`,
      createTime: new Date(Date.now() - (index + 10) * 86400000),
      approveTime: new Date(Date.now() - index * 86400000),
      reason: `这是变更原因说明，详细描述了为什么需要进行此次变更。原因内容${index + 1}`,
      content: `这是变更内容说明，详细描述了本次变更的具体内容和涉及的课程。变更内容${index + 1}`,
      approveComment: `同意此次变更申请，变更内容符合专业培养要求。`,
